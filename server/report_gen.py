...
def generate_full_report_pdf(stages: Dict[str, dict], design_name: str,
                             build_status: str, events: List[dict],
                             all_generated_files: List[dict] = None) -> bytes:
    """Return PDF bytes for the full build report (all stages).
    
    Args:
        stages: Dict of stage_name -> stage data (for completed stages)
        design_name: Name of the design
        build_status: 'success' or 'failed'
        events: List of build events
        all_generated_files: Optional list of all files generated in workspace
    """
    ...
    # All Generated Files section
    if all_generated_files:
        _ensure_space(80)
        y = _pdf_section(page, y, "All Generated Files")
        for f in all_generated_files:
            _ensure_space(30)
            fname = f.get("name", f.get("path", "unknown"))
            fsize = f.get("size", 0)
            fpath = f.get("path", "")
            y = _pdf_body(page, y, f"• {fname}", font="hebo", size=9)
            if fsize:
                y = _pdf_body(page, y, f"  Size: {fsize} bytes",
                              size=8, color=(0.4, 0.4, 0.4), indent=46)
            if fpath:
                y = _pdf_body(page, y, f"  Path: {fpath[:100]}",
                              size=8, color=(0.4, 0.4, 0.4), indent=46)
...
def generate_full_report_docx(stages: Dict[str, dict], design_name: str,
    ...
    if all_generated_files:
        doc.add_page_break()
        _docx_heading(doc, "All Generated Files")
        for f in all_generated_files:
            fname = f.get("name", f.get("path", "unknown"))
            fsize = f.get("size", 0)
            fpath = f.get("path", "")
            ...
            run.bold = True
            if fsize:
                doc.add_paragraph(f"   Size: {fsize} bytes").runs[0].font.size = Pt(8)
            if fpath:
                doc.add_paragraph(f"   Path: {fpath[:200]}").runs[0].font.size = Pt(8)
