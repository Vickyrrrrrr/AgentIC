...
MANDATORY RTL RULES (violations will cause synthesis errors — never break these):
  ...
  • CPU SoC Integration: If using PicoRV32 or any CPU core, expose and connect the real memory interface signals (`mem_valid`, `mem_addr`, `mem_wdata`, `mem_wstrb`, `mem_rdata`, `mem_ready`). NEVER collapse address/data/write-strobe into one generic `proc_out` bus. NEVER drive UART/GPIO/PWM/watchdog controls directly from CPU address bits. Build an address decoder, memory-mapped registers, and a boot ROM or firmware loader path.
