...
def _find_openlane_gds(design_dir: str, run_tag: str, design_name: str) -> str:
    """Find a GDS generated by either OpenLane 1 or OpenLane 2."""
    candidate_patterns = [
        os.path.join(design_dir, "runs", run_tag, "results", "final", "gds", f"{design_name}.gds"),
        os.path.join(design_dir, "runs", run_tag, "final", "**", f"{design_name}.gds"),
        os.path.join(design_dir, "runs", run_tag, "**", f"{design_name}.gds"),
    ]
...
def _tcl_env_value(config_tcl: str, key: str, default: str = "") -> str:
    ...
    return bare.group(1).strip() if bare else default
...
def _tcl_words(value: str) -> List[str]:
    ...
    list_match = re.match(r"^\[list\s+(.+)\]$", value)
    if list_match:
...
def _design_relative_path(value: str, design_dir: str) -> str:
    value = str(value or "").strip().strip('"')
    value = value.replace("$::env(DESIGN_DIR)", design_dir)
    if os.path.isabs(value) and value.startswith(design_dir):
        return "dir::" + os.path.relpath(value, design_dir).replace(os.sep, "/")
    return value


def _openlane2_verilog_files(design_dir: str, design_name: str) -> List[str]:
    src_dir = os.path.join(design_dir, "src")
    preferred = os.path.join(src_dir, f"{design_name}.v")
    if os.path.exists(preferred):
        return [f"dir::src/{design_name}.v"]
    ...
    for path in sorted(glob.glob(os.path.join(src_dir, "*.v")) + glob.glob(os.path.join(src_dir, "*.sv"))):
        ...
        if any(base.endswith(suffix) for suffix in excluded_suffixes):
            continue
        files.append("dir::" + os.path.relpath(path, design_dir).replace(os.sep, "/"))
    return files or ["dir::src/*.v"]


def _write_openlane2_config(
    design_name: str,
    design_dir: str,
    config_tcl: str,
    pdk_name: str,
) -> str:
    """Translate AgentIC's v1-style config.tcl essentials into OpenLane 2 JSON."""
    std_cell_library = _tcl_env_value(config_tcl, "STD_CELL_LIBRARY", "")
    clock_port = _tcl_env_value(config_tcl, "CLOCK_PORT", "clk")
    clock_net = _tcl_env_value(config_tcl, "CLOCK_NET", clock_port)
    clock_period = _as_number(_tcl_env_value(config_tcl, "CLOCK_PERIOD", "10"), 10.0)
    die_area_raw = _tcl_env_value(config_tcl, "DIE_AREA", "0 0 500 500")
    die_numbers = [_as_number(token, 0.0) for token in re.findall(r"-?\d+(?:\.\d+)?", die_area_raw)]
    die_area = die_numbers[:4] if len(die_numbers) >= 4 else [0, 0, 500, 500]

    config: Dict[str, Any] = {
        "DESIGN_NAME": design_name,
        "VERILOG_FILES": _openlane2_verilog_files(design_dir, design_name),
        "CLOCK_PORT": clock_port,
        "CLOCK_NET": clock_net,
        "CLOCK_PERIOD": clock_period,
        "FP_SIZING": "absolute",
        "DIE_AREA": die_area,
        "FP_CORE_UTIL": int(_as_number(_tcl_env_value(config_tcl, "FP_CORE_UTIL", "40"), 40)),
        "PL_TARGET_DENSITY": _as_number(_tcl_env_value(config_tcl, "PL_TARGET_DENSITY", "0.45"), 0.45),
        "SYNTH_STRATEGY": _tcl_env_value(config_tcl, "SYNTH_STRATEGY", "AREA 0"),
        "MAX_FANOUT_CONSTRAINT": int(_as_number(_tcl_env_value(config_tcl, "MAX_FANOUT_CONSTRAINT", "8"), 8)),
    }
    if pdk_name:
        config["PDK"] = pdk_name
    if std_cell_library:
    ...
    sdc = _tcl_env_value(config_tcl, "SDC_FILE", "") or _tcl_env_value(config_tcl, "BASE_SDC_FILE", "")
    if sdc:
        sdc = _design_relative_path(sdc, design_dir)
        config["PNR_SDC_FILE"] = sdc
        config["SIGNOFF_SDC_FILE"] = sdc

    for key in ("EXTRA_LEFS", "EXTRA_GDS_FILES", "EXTRA_LIBS", "VERILOG_FILES_BLACKBOX"):
        values = _tcl_words(_tcl_env_value(config_tcl, key, ""))
        if values:
            config[key] = [_design_relative_path(value, design_dir) for value in values]

    macro_placement = _tcl_env_value(config_tcl, "MACRO_PLACEMENT_CFG", "")
    if macro_placement:
        config["MACRO_PLACEMENT_CFG"] = _design_relative_path(macro_placement, design_dir)

    for key in ("VDD_NETS", "GND_NETS"):
        values = _tcl_words(_tcl_env_value(config_tcl, key, ""))
        if values:
            config[key] = values

    pdn_hooks = _tcl_env_value(config_tcl, "FP_PDN_MACRO_HOOKS", "")
    if pdn_hooks:
        config["FP_PDN_MACRO_HOOKS"] = pdn_hooks

    for key in ("PL_MACRO_HALO", "PL_MACRO_CHANNEL"):
        values = [_as_number(value, 0) for value in _tcl_words(_tcl_env_value(config_tcl, key, ""))]
        if values:
            config[key] = values

    macro_blockages = _tcl_env_value(config_tcl, "MACRO_BLOCKAGES_LAYER", "")
    if macro_blockages:
        config["MACRO_BLOCKAGES_LAYER"] = macro_blockages
    ...
    for key in (
        ...
        "GRT_RESIZER_TIMING_OPTIMIZATIONS",
    ):
        value = _tcl_env_value(config_tcl, key, "")
        if value != "":
            config[key] = _as_number(value, value)

    config_path = os.path.join(design_dir, "config.openlane2.json")
...
def _run_openlane2(
    ...
    """Run OpenLane 2 through its official non-interactive Dockerized wrapper."""
    ...
    if not os.path.exists(config_tcl_path):
        return False, f"OpenLane config not found: {config_tcl_path}"
    try:
        with open(config_tcl_path, "r", encoding="utf-8", errors="replace") as f:
            config_tcl = f.read()
        tool_config = get_pdk_tool_config(selected_pdk)
        std_cell_library = _tcl_env_value(config_tcl, "STD_CELL_LIBRARY", "") or str(
            tool_config.get("std_cell_library", "")
        )
        config_json = _write_openlane2_config(design_name, design_dir, config_tcl, selected_pdk)
    except Exception as exc:
        return False, f"Failed to generate OpenLane 2 config: {exc}"
    ...
    if std_cell_library:
        cmd.extend(["-s", std_cell_library])
    cmd.append(config_json)

    env = os.environ.copy()
    env["PDK_ROOT"] = effective_pdk_root
    env["PDK"] = selected_pdk
    env["OPENLANE_IMAGE"] = OPENLANE2_IMAGE

    if background:
        log_file_path = os.path.join(design_dir, "harden.log")
        try:
            with open(log_file_path, "w", encoding="utf-8") as f:
                subprocess.Popen(cmd, stdout=f, stderr=subprocess.STDOUT, cwd=design_dir, env=env, start_new_session=True)
            return True, f"Background OpenLane 2 task started. Logs: {log_file_path}"
        except Exception as e:
            return False, f"Failed to start OpenLane 2 background process: {str(e)}"
    ...
    try:
        ...
        return False, "OpenLane 2 hardening timed out (exceeded 60 mins)."
    except (OSError, FileNotFoundError) as e:
        return False, f"OpenLane 2 command failed: {str(e)}."

    gds_path = _find_openlane_gds(design_dir, run_tag, design_name)
    if process.returncode == 0 and gds_path:
        return True, gds_path

    error_text = (process.stdout + "\n" + process.stderr).strip()
    if process.returncode == 0 and not gds_path:
        return False, error_text or "OpenLane 2 finished but did not produce a GDS."
